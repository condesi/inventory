...
class Warehouse_model extends App_Model {
    ...
    /**
     * Guarda un registro en caché
     */
    private function cache_set($key, $value, $expiration = 3600) {
        return warehouse_cache_set('wh_' . $key, $value, $expiration);
    }
    
    /**
     * Obtiene un registro de caché
     */
    private function cache_get($key) {
        return warehouse_cache_get('wh_' . $key);
    }

    /**
     * Obtener almacenes con filtros y estadísticas
     * @param array $filters
     * @return array
     */
    public function get_warehouses($filters = []) {
        $cache_key = 'warehouses_' . md5(serialize($filters));
        ...
        if (!empty($filters['search'])) {
            ...
            $this->db->or_like('w.code', $filters['search']);
            $this->db->group_end();
        }
        
        // Aplicar permisos
        $warehouseByStaff = $this->getWarehouseByStaff();
        if(is_array($warehouseByStaff)){
            $this->db->where_in('w.id', $warehouseByStaff);
        }
   ...
   public function update_po_detail_quantity($po_id, $goods_receipt_detail)
    ...
    public function current_stock_report_pdf($data)
    {
        ...
        // Add a page
        $pdf->AddPage();
        $pdf->SetFont(get_pdf_font_name(), '', get_option('pdf_font_size'));
        ...
        // Get the report view
        $html = $CI->load->view('warehouse/report/current_stock_report_pdf', [
            'stock_data' => $stock_data,
            'title' => _l('current_stock_report'),
    ...
    /**
     * Get current stock data for reports
     * @param  mixed $warehouse_filter
     * @param  mixed $commodity_filter
     * @param  mixed $group_filter
     * @param  mixed $status_filter
     * @return array
     */
    public function get_current_stock_data($warehouse_filter = null, $commodity_filter = null, $group_filter = null, $status_filter = null)
    {
        $this->db->select('i.*, c.commodity_code, c.description, c.commodity_name, c.unit_id, u.unit_name, w.warehouse_name, g.name as group_name');
        $this->db->from(db_prefix() . 'inventory_manage i');
        $this->db->join(db_prefix() . 'items c', 'c.id = i.commodity_id', 'left');
        $this->db->join(db_prefix() . 'ware_unit_type u', 'u.unit_type_id = c.unit_id', 'left');
        $this->db->join(db_prefix() . 'warehouse w', 'w.warehouse_id = i.warehouse_id', 'left');
        $this->db->join(db_prefix() . 'items_groups g', 'g.id = c.group_id', 'left');
        
        // Apply filters
        if (!empty($warehouse_filter)) {
            if (is_array($warehouse_filter)) {
                $this->db->where_in('i.warehouse_id', $warehouse_filter);
            } else {
                $this->db->where('i.warehouse_id', $warehouse_filter);
            }
        }
        
        if (!empty($commodity_filter)) {
            if (is_array($commodity_filter)) {
                $this->db->where_in('i.commodity_id', $commodity_filter);
            } else {
                $this->db->where('i.commodity_id', $commodity_filter);
            }
        }
        
        if (!empty($group_filter)) {
            if (is_array($group_filter)) {
                $this->db->where_in('c.group_id', $group_filter);
            } else {
                $this->db->where('c.group_id', $group_filter);
            }
        }
        
        if (!empty($status_filter)) {
            if ($status_filter == 'warning') {
                $this->db->where('i.inventory_number <= c.minimum_stock');
                $this->db->where('i.inventory_number > 0');
        ...
        $this->db->order_by('c.commodity_code', 'asc');
